...
async function choose(item){if(!target)return;const file=item.blob?new File([item.blob],item.name,{type:item.type||item.blob.type}):await blobFromUrl(item.url,item.filename);setInputFile(target,file);close();}
function css(){if(document.querySelector('#h3libcss'))return;const s=document.createElement('style');s.id='h3libcss';s.textContent='html{font-size:15px}body{font-size:1rem}h2{font-size:1rem}label{font-size:.8rem}.small{font-size:.75rem}.notice{font-size:.8rem}.pill,.mode button,.primary,.secondary,.nav,input,select,textarea,summary{font-size:.94rem}.h3lib-btn{margin-top:8px;padding:9px 10px!important}.h3lib-modal{position:fixed;inset:0;background:#000c;z-index:80;padding:18px;overflow:auto;overflow-x:hidden;max-width:100vw}.h3lib-box{width:100%;max-width:720px;min-width:0;margin:5vh auto;background:#171a21;border:1px solid #2b3240;border-radius:16px;padding:14px;overflow:hidden}.h3lib-grid{display:grid;width:100%;min-width:0;grid-template-columns:repeat(3,minmax(0,1fr));gap:9px}.h3lib-item{min-width:0;max-width:100%;overflow:hidden;background:#11151c;border:1px solid #2b3240;border-radius:12px;padding:7px}.h3lib-item img{display:block;width:100%;max-width:100%;min-width:0;aspect-ratio:1;object-fit:cover;border-radius:8px}.h3lib-item .name{max-width:100%;min-width:0;font-size:10px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;margin:5px 0}.h3lib-item button{width:100%;min-width:0;max-width:100%;padding:7px;margin-top:4px;font-size:.8rem;white-space:normal;overflow-wrap:anywhere}.h3lib-tabs{display:flex;min-width:0;gap:6px;margin:8px 0 12px}.h3lib-tabs button{min-width:0;font-size:.8rem}@media(max-width:460px){.h3lib-modal{padding:12px}.h3lib-box{padding:11px}.h3lib-grid{grid-template-columns:repeat(2,minmax(0,1fr));gap:7px}}';document.head.appendChild(s);}
function close(){document.querySelector('#h3libmodal')?.remove();}
async function render(mode='uploaded'){const body=document.querySelector('#h3libbody');if(!body)return;body.innerHTML='<div class="small">読込中...</div>';let items=[];if(mode==='uploaded'){const r=await fetch('/h3-mobile/api/input-images');const j=await r.json();items=j.images||[];}else items=await allFav();body.innerHTML=items.length?'':'<div class="small">まだありません。</div>';const g=document.createElement('div');g.className='h3lib-grid';for(const it of items){const d=document.createElement('div');d.className='h3lib-item';const url=it.url||(it.blob?URL.createObjectURL(it.blob):'');d.innerHTML=`<img src="${url}"><div class="name">${it.filename||it.name}</div><button class="secondary use">使う</button>${mode==='uploaded'?'<button class="secondary fav">☆ お気に入り</button>':'<button class="secondary del">削除</button>'}`;d.querySelector('.use').onclick=()=>choose(it);if(mode==='uploaded')d.querySelector('.fav').onclick=async()=>{try{const f=await blobFromUrl(it.url,it.filename);await putFav({id:crypto.randomUUID(),name:it.filename,type:f.type,blob:f,created:Date.now()});alert('お気に入りに保存しました');}catch(e){alert(e.message);}};else d.querySelector('.del').onclick=async()=>{await delFav(it.id);render('favorites');};g.appendChild(d);}body.appendChild(g);}
